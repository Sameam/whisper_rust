use crate::{models::silero::Silero, utils::utils::{VadParams, TimeStamp}};
...
impl VadIter {
    pub fn new(silero: Silero, mut params: VadParams) -> Self {
        params.sample_rate  = params.sample_rate;
        Self { silero, params: Params::from(params), state: State::new(), speeches: Vec::new() }
    }

    pub fn process(&mut self, frame: &[i16]) -> Result<bool, ort::Error> {
        // 1) get Silero’s probability for this slice
        let prob: f32 = self.silero.calc_level(frame)?;
        // 2) update the FSM (this bumps current_sample, sets triggered, etc)
        self.state.update(&self.params, prob);

        let segment: &Vec<TimeStamp> = self.speeches();

        log::info!("Time stamp for the chunk: {:?}", segment);
        // 3) return whether we’re currently in a speech segment
        Ok(self.state.triggered)
    }
...
impl From<VadParams> for Params {
    fn from(value: VadParams) -> Self {
        ...
        let max_speech_samples = sample_rate as f32 * max_speech_duration_s - frame_size_samples as f32  - 2.0 * speech_pad_samples as f32;
...
impl State {
    ...
    fn update(&mut self, params: &Params, speech_prob: f32) {
        ...
        if self.triggered && (self.current_sample as i64 - self.current_speech.start) as f32 > params.max_speech_samples {
            if self.prev_end > 0 {
